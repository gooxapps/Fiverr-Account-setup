...
function EmptyState({ onGenerate }: { onGenerate: (s: string) => void }) {
  const examples = ["Flutter app developer", "Logo designer", "SEO specialist", "Video editor"];
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-6 relative z-10">
      <div className="w-28 h-28 rounded-full bg-gradient-to-br from-violet-600/30 to-blue-600/30 border border-violet-500/30 flex items-center justify-center mb-8 shadow-2xl shadow-violet-500/20 backdrop-blur-xl relative">
        <div className="absolute inset-0 rounded-full border border-white/10 scale-110" />
        <svg className="w-12 h-12 text-violet-300 drop-shadow-md" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
      </div>
      <h3 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 mb-4 tracking-tight">Generate Your First Gig</h3>
      ...
      <div className="flex flex-wrap gap-3 justify-center mb-12">
        {examples.map((ex) => (
          <button
            ...
            className="px-5 py-2.5 rounded-full bg-white/5 border border-white/10 text-sm font-medium text-slate-200 hover:border-violet-500/50 hover:bg-violet-500/15 hover:text-white transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-violet-500/10"
          >
            Try: {ex}
          </button>
        ))}
